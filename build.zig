...
fn emLinkStep(b: *Build, options: EmLinkOptions) !*Build.Step.Run {
    ...
    if (options.use_webgl2) {
        ...
        try emcc_cmd.append("-sMIN_WEBGL_VERSION=2");
    }
    ...
    try emcc_cmd.append("-s SDL2_IMAGE_FORMATS='[\"bmp\",\"png\"]'");
    try emcc_cmd.append("-s USE_SDL_IMAGE=2");

    try emcc_cmd.append("--use-port=sdl2_image:formats=bmp,png,xpm,jpg");
    try emcc_cmd.append("--use-port=sdl2");
